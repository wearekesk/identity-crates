...
pub fn encrypt_s(k_enc: &[u8], s: &[u8]) -> Result<Vec<u8>, BacError> {
    ...
    cipher.encrypt(s, false).map_err(|e| BacError(e.to_string()))
}
...
mod tests {
    ...
    fn generate_s_concatenates_fields() {
        ...
        assert_eq!(&s[NONCE_LEN..2 * NONCE_LEN], hex("4608F91988702212").as_slice());
    }
    ...
    fn icao_d3_full_ea_response_roundtrip() {
        ...
        assert_eq!(hex::encode_upper(&ks.first), "979EC13B1CBFE9DCD01AB0FED307EAE5");
        assert_eq!(hex::encode_upper(&ks.second), "F1CB1F1FB5ADF208806B89DC579DC1F8");
